...
async def yt_dl_video(client, cb):
    ...
    if audio_or_video == "video":
        file_ = InputMediaVideo(file_name, thumb=downloaded_thumb, supports_streaming=True, duration=dur, caption = f"**{type_.title()} Name ➠** `{vid_title}`")
    else:
        file_ = InputMediaAudio(file_name, performer=uploader, title=name, thumb=downloaded_thumb, duration=dur, caption = f"**{type_.title()} Name ➠** `{vid_title}`")
