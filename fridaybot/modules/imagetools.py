...
from fridaybot.function import runcmd, convert_to_image, crop_vid, take_screen_shot#    Copyright (C) Midhun KM 2020
#    This program is free software: you can redistribute it and/or modify
#    it under the terms of the GNU Affero General Public License as published by
#    the Free Software Foundation, either version 3 of the License, or
#
...
async def spinshit(message):
    ...
    if not reply:
        ...
        step = 1
    pic_loc = await media_to_image(message.media)
    if not pic_loc:
        await message.edit("Reply to a valid media first")
        return
    await message.edit("🌀 `Tighten your seatbelts, sh*t is about to get wild ...`")
